...
public class RobotSim implements Runnable {
  ...
  public void run() {
    ...
    testPub.set(
        new Pose3d(
            new Translation3d(testX.get(), testY.get(), testZ.get()),
    ...
    elevatorPub.set(
        new Pose3d()
            .transformBy(
                new Transform3d(0., 0., Units.inchesToMeters(totalHeight / 2), Rotation3d.kZero)));
    carriagePub.set(
        new Pose3d()
            .transformBy(
                new Transform3d(0., 0., Units.inchesToMeters(totalHeight), Rotation3d.kZero)));

    // initialize the claw pose
    // the claw has to be positioned so the shaft rotates around the axises, then transposed up to
    // the right location
    var clawPose = new Pose3d();
    clawPose =
        clawPose.transformBy(
            new Transform3d(
                0.2667,
                0,
                Units.inchesToMeters(heightSub.get()),
    ...
    coral =
        coral.transformBy(
            new Transform3d(
                ...
                0,
                Units.inchesToMeters(11 + heightSub.get()),
    ...
    // NOTE: Pose3d.plus does NOT work here, as it rotates you around a piece origin, we just want
    // to add translations
    // coral = coral.plus(new Transform3d(drivePose.getX(), drivePose.getY(), 0, Rotation3d.kZero));
    coral =
        new Pose3d(coral.getTranslation().plus(drivePose.getTranslation()), coral.getRotation());
