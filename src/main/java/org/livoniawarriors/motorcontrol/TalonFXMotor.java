...
public class TalonFXMotor extends MotorControl {
  ...
  public TalonFXMotor(String motorName, TalonFX motor, boolean logOnly) {
    ...
    // configure all signals for our rate (we pick 18 milliseconds to always have fresh data for
    // 20ms loops)
  ...
  public void setBrakeMode(boolean brakeOn) {
    var statusCode =
        motor.setNeutralMode(brakeOn ? NeutralModeValue.Brake : NeutralModeValue.Coast);
  }
  ...
  /**
   * CTRE returns positions as motor revolutions, this lets us turn motor revolutions into something
   * useful.
   */
  @Override
  ...
  public double getRpm() {
    var statusCode = motorRpm.refresh().getStatus();
    // internally the motor runs in rev/sec, but return an WpiLib unit type.  We are using WpiLib to
    // scale it for us.
  ...
  public void setVelocity(double velocity) {
    var statusCode =
        motor.setControl(
            velocitySetter.withVelocity(velocity).withAcceleration(pidConstants.kAccelMax));
    // atSetpoint
  }
  ...
  public void setEncoderPosition(double position) {
    var statusCode =
        cfg.setPosition(
            Angle.ofBaseUnits(position * 2 * Math.PI, Units.Rotations).in(Units.Rotations));
  }
  ...
  public boolean isConnected() {
    ...
    var statusCode = motorTemp.refresh().getStatus();
    return statusCode != StatusCode.EcuIsNotPresent
        && statusCode != StatusCode.RxTimeout
        && statusCode != StatusCode.CanMessageStale;
  }
