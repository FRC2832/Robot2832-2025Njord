...
public class PidConstants {
  ...
  /**
   * How much voltage do we add based on amount of error from set point. For example, if P=2, that
   * means for each input unit we are off, add 2 volts to the output.
   *
   * <p>Units: Volts/error
   */
  public double kP;
  ...
  /**
   * How much voltage do we add based on the sum of error. This number should be small!!! For
   * example, if I=2, for each input unit we are off for 1 sec, add 2 volts to the output.
   *
   * <p>Units: Volts/sum of error
   */
  public double kI;
   ...
   *
   * <p>Units: Volts / Rev/S^2
   */
  public double kA;

  /**
   * How much range you want before the I-Term enables. For example, if kiZone=10, and the
   * measurement is 12 units off, ignore the I term. OK to start at zero, this is used to stop
   * integral windup when the mechanism starts moving and usually adds more overshoot after reaching
   * the target.
   *
   * <p>Units: Distance
   */
  ...
  /**
   * How much range is allowable to turn off the PID. For example, if kiError = 2, and the mechanism
   * is 1 unit away, the PID would turn off. OK to start at zero. This has a side effect of
   * basically always stopping control the iError off, so probably good to keep off.
   *
   * <p>Units: Distance
   */
  ...
  public PidConstants(String key, boolean persist) {
    ...
    // NOTE: Remote does not work in the Simulation dashboard, need to use external dashboard like
    // Elastic or Shuffleboard
